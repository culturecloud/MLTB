...
from bot import config_dict, user_data, DATABASE_URL, DATABASE_NAME, MAX_SPLIT_SIZE, DRIVES_IDS, DRIVES_NAMES, INDEX_URLS, aria2, GLOBAL_EXTENSION_FILTER, status_reply_dict_lock, Interval, aria2_options, aria2c_global, IS_PREMIUM_USER, download_dict, qbit_options, get_client, LOGGER, bot
...
async def load_config():
    ...
    if len(BASE_URL) == 0:
        ...
        await create_subprocess_shell(f"gunicorn web.server:app --bind 0.0.0.0:{SERVER_PORT} --logger-class=web.log_config.StubbedGunicornLogger")
...
async def get_buttons(key=None, edit_type=None):
    ...
    if key is None:
        ...
        if key in ['SUDO_USERS', 'RSS_USER_SESSION_STRING', 'CMD_SUFFIX', 'OWNER_ID', 'USER_SESSION_STRING',
                   'TELEGRAM_HASH', 'TELEGRAM_API', 'AUTHORIZED_CHATS', 'RSS_DELAY', 'DATABASE_URL', 'DATABASE_NAME',
...
async def edit_variable(client, message, pre_message, key):
    ...
    if value.lower() == 'true':
        ...
        value = int(value)
        await (await create_subprocess_exec("pkill", "-9", "-f", "gunicorn")).wait()
        await create_subprocess_shell(f"gunicorn web.server:app --bind 0.0.0.0:{value} --logger-class=web.log_config.StubbedGunicornLogger")
...
async def update_private_file(client, message, pre_message):
    ...
    if not message.media and message.text:
        ...
        await message.download(file_name=f'/culturecloud/mltb/{file_name}')
...
async def edit_bot_settings(client, query):
    ...
    if data[1] == 'close':
        ...
        if data[2] in default_values:
            ...
            value = 80
            await (await create_subprocess_exec("pkill", "-9", "-f", "gunicorn")).wait()
            await create_subprocess_shell("gunicorn web.server:app --bind 0.0.0.0:80 --logger-class=web.log_config.StubbedGunicornLogger")
