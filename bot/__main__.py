...
from bot import bot, bot_loop, config_dict, botStartTime, LOGGER, Interval, DATABASE_URL, user, QbInterval, INCOMPLETE_TASK_NOTIFIER, scheduler
from .helper.ext_utils.fs_utils import start_cleanup, clean_all
...
async def start(client, message):
    buttons = ButtonMaker()
    buttons.ubutton("Repo", "https://cutt.ly/03xEtD0")
    buttons.ubutton("Owner", "https://t.me/pseudokawaii")
    reply_markup = buttons.build_menu(2)
    if await CustomFilters.authorized(client, message):
        start_string = f'''
**⚡ Welcome to MLTB!**

The all-in-one Telegram bot which utilizes some of the world's most awesome & powerful file transfer tools like Aria2, qBittorrent, YTDLP and help you do ✨ <i>stuffs</i> ✨

<i>See /{BotCommands.HelpCommand} for more info on how to use me.</i>
        '''
        await sendMessage(message, start_string, reply_markup)
    else:
        await sendMessage(message, "Sorry, I'm not interested in NTR. <spoiler>Here, have a banana 🍌</spoiler>", reply_markup)
...
async def bot_help(client, message):
    sendMessage(message, help_string)
...
if config_dict['SET_BOT_COMMANDS']:
    botcmds = [
        ...
        (f'{BotCommands.RssCommand}','RSS Menu'),
        (f'{BotCommands.ShellCommand}','Run shell commands (Owner)'),
        (f'{BotCommands.EvalCommand}','Run Python code line | lines (Owner)'),
        (f'{BotCommands.ExecCommand}','Run commands in exec (Owner)')
    ]
